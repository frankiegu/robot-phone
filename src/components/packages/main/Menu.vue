    ...
    <div class="nav" ref="nav">
      <ul v-if="menuList.length" id="menuUl">
        ...
        >
          ...
          >
            <svg class="iconSmbol" aria-hidden="true">
          ...
          <collapse-transition v-if="item.children && item.children.length">
            <ul class="nav-child" v-show="activeIndex[0] === i">
              <li
              ...
              >
                <a
                  ...
                  @click.prevent="handleMenuSelect(child, [i, j])"
                >{{ child.title }}</a>
              </li>
            </ul>
          </collapse-transition>
...
import { navActiveIndexSession } from "@/storage/session";
...
export default {
  ...
  computed: {
    ...mapGetters(["menuList", "currentPageName", "system"])
  },
  watch: {
    $route() {
      this.$nextTick(() => {
      ...
      if (this.currentPageName !== item.name) {
        if (activeIndex.length >= this.activeIndex.length) {
          ...
          this.activeIndex = activeIndex.filter((v, i) => {
            return (
              !this.activeIndex[i] ||
              (this.activeIndex[i] && this.activeIndex[i] !== v)
            );
          });
        }
        ...
        if (
          name &&
          (name === Util.currentPageName(this.system) ||
