<template>
  <div>
    <Form inline>
      <FormItem label="学习状态" :label-width="80">
        <Select
          style="width: 120px;"
    ...
    <Table :stripe="true" :columns="table.columns" :data="table.list" ref="table"/>
    <ms-pagination
    ...
    <call-record-detail-panel :id="callRecord.id" v-model="callRecord.show"/>

    <ms-panel v-model="study.show">
      <div slot="header" style="margin-right: 100px;">
        学习问题: {{study.entity.content}}({{study.entity.createTime | date}})
        <Button type="success" size="small" @click="updateStatusStudy" class="ml-20">标为已学习</Button>
        <Button type="primary" size="small" @click="showCallRecord(study.entity)">通话记录</Button>
      </div>
            ...
            render: (h, { row }) => {
              ...
              return(             
                <span>    
                  <a
                    href="javascript: void(0);"
                    onClick={() => this.showCallRecord(row)}
                  >
                    通话记录
                  </a>
                  {btns}
