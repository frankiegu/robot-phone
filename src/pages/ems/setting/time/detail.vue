<template>
  <div class="task-detail">
    <div v-show="!!templateId">
      <div class="cf">
        <div class="dib">
          ...
          <sub>ID: {{template.id}}</sub>
        </div>
        <div class="fr">
          <div class="mb10">状态：{{statusMap[template.status]}}</div>
          <i-switch v-model="template.status" size="large" :true-value="1" :false-value="0" @on-change="handlerChangeStatus">
      ...
      <div class="mt20">
        ...
        <div class="mb10 cf">
          <span class="fl dib vm">拨号时间：</span>
          <div class="ml70">
            <i-button type="primary" class="mr15 mb15" v-for="(item,i) in weekDate" :key="i">{{weekArray[item - 1]}}</i-button>
          </div>
        </div>
        <div class="mb10 cf">
          <span class="fl dib vm">排除日期：</span>
          <div class="ml70">
            <div>
        ...
        <div class="mb10 cf">
          <div class="ml70">
            <i-button type="text" size="small" @click="handlerShowExpire">
              已过期日期({{timeOutDate.length}})
              <Icon :type="showExpire ? 'chevron-down': 'chevron-up'"></Icon>
            </i-button>
            <transition name="slide-down">
              <div v-show="showExpire">
    ...
    <!-- 没数据 -->
    <div v-show="!templateId" class="ptb50 tc"> <span>暂无数据</span> </div>
  </div>
</template>
<script>
const template = {
