<template>
    <Form ref="form" :model="entity" :rules="rules" :label-width="90">
      <div class="fleX" style="height:28rem;overflow:auto;">
              <div>
        <div class="board mr-20">
          <div class="board-header">企业信息</div>
          <div class="board-body">
            <FormItem label="开通企业" prop="adminId">
              <i-select v-model="entity.adminId" placeholder="请选择企业">
                <i-option
            ...
            <FormItem label="坐席单价" prop="perPrice">
              <i-input v-model="entity.perPrice" placeholder="请输入单价">
                <span slot="append">元</span>
              </i-input>
            </FormItem>

            <FormItem label="市级代理:">{{entity.name}}</FormItem>

            <FormItem label="剩余年端口:">{{entity.remainPort}}</FormItem>
            <FormItem label="剩余月端口:">{{entity.monthCardRemainCount}}</FormItem>
            <FormItem label="购买类型" prop="portType">
              <i-select v-model="entity.portType" placeholder="请选择购买类型">
                <i-option value="0">购买年端口</i-option>
                <i-option value="1">购买月端口</i-option>
              </i-select>
            </FormItem>
            <FormItem label="购买期限" prop="serviceLife">
              <i-input v-model="entity.serviceLife" placeholder="请输入购买值">
                <span slot="append">{{typeString}}</span>
              </i-input>
            </FormItem>
          </div>
          <div class="board-header">通话信息</div>
          <div class="board-body">
            <FormItem label="快递名称" prop="expressCompany">
              <i-select v-model="entity.expressCompany" placeholder="请选择快递">
                <i-option v-for="item in expressList" :key="item.id" :value="item.id">{{item.name}}</i-option>
              </i-select>
            </FormItem>
            <FormItem label="快递单号" prop="expressNo">
              <i-input v-model="entity.expressNo" placeholder="请输入快递单号"/>
            </FormItem>
          </div>
        </div>
      </div>
      <div>
        <div class="board" style="box-shadow:none;">
          ...
          <div class="board-body">
            <div>
              <div class="text-r mb-5">
                <Button  @click="handlerAddSeat(1)" v-show="!seatId">新增</Button>
              </div>
              <div>
                <div class="seat">
                  <FormItem
                    v-for="(item,i) in entity.robotSeats"
                    :key="i"
                    class="cn"
                    :prop="'robotSeats.' + i"
                    :rules="rules.seatNum"
          ...
          <div class="board-body">
            <div>
              <div class="text-r mb-5">
                <Button  @click="handlerAddSeat(2)" v-show="!seatId">新增</Button>
              </div>
              <div>
                <div class="seat">
                  <FormItem
                    v-for="(item,i) in entity.humanSeats"
                    :key="i"
                    class="cn"
                    :prop="'humanSeats.' + i"
                    :rules="rules.seatNum"
          ...
          <div class="board-header">
            开通固定坐席
            <span class="vma-info">(只支持三汇设备)</span>
            <span class="fr">合计数量: {{entity.fixedSeats.length}}</span>
          </div>
          <div class="board-body">
            <div>
              <div class="text-r mb-5">
                <Button  @click="handlerAddSeat(3)" v-show="!seatId">新增</Button>
              </div>
              <div>
                <div class="seat">
                  <FormItem
                    v-for="(item,i) in entity.fixedSeats"
                    :key="i"
                    class="cn"
                    :prop="'fixedSeats.' + i"
                    :rules="rules.seatNum"
          ...
          <!-- <div class="tc" v-show="!seatId">
            <i-button type="text" @click="handlerCancel">取消</i-button>
...
export default {
  ...
  data() {
    ...
    return {
      entity: {
        ...entity,
        ...{ adminId: this.adminId, humanSeats: [], robotSeats: [] }
      },
      rules: {
