<template>
  <div class="task">
    <div class="task-l">
      <div class="board board-primary">
        ...
        <div class="board-body">
          <whispering-flow-item title="主流程"
                                :list="node.main.list"
          ...
          <whispering-flow-item title="旁白流程"
          ...
          <whispering-flow-item title="关键字结束流程"
                                :list="node.end.list"
                                :currentNode="current.entity"
                                :showSort="false"
          ...
          <whispering-flow-item title="挽回结束流程"
          ...
          <whispering-flow-item title="无人应答流程"
          ...
          <whispering-flow-item title="未匹配关键字流程"
          ...
          <whispering-flow-item title="转接人工流程"
    ...
    <div class="task-r">
      <div class="board board-primary">
        <div class="board-body">
          <whispering-node :data="current.entity"
                           ...
                           ref="whisperingNode" />
          <p class="nodata"
             v-else>请选择话术节点或创建新的话术节点</p>
        </div>
      </div>
    </div>
    <Modal v-model="sort.show"
           :title="sort.title">
      <sortable-node :list="sort.list"
                     @change="sortList" />
      <div slot="footer"></div>
    </Modal>
  </div>
...
import whisperingNodeApi from '@/api/sms/whisperingNode'
...
export default {
  ...
  data() {
    ...
    return {
      node: {
        ...
        // 挽回结束流程
        redeemEnd: new FlowItem({ getType: 4, nodeType: 7, title: '挽回结束流程' }),
        // 无人应答流程
        noResponse: new FlowItem({ getType: 5, nodeType: 8, title: '无人应答流程' }),
        // 未匹配关键字流程
        unmatch: new FlowItem({ getType: 6, nodeType: 9, title: '未匹配关键字流程' }),
        // 转接人工流程
        manual: new FlowItem({ getType: 7, nodeType: 10, title: '转接人工流程' })
      },
      current: {
        // 当前流程数据
        entity: null
  ...
  methods: {
    ...
    couldChangeWhisperingNode() {
      if (this.current.entity && this.$refs.whisperingNode && this.$refs.whisperingNode.hasModify()) {
        return false
      }
      return true
    },
