...
import { UnrelateFirmSelect } from '@/components/packages/sms/select'
  ...
  methods: {
    ...
    submitRelate() {
      this.$refs.relateForm.validate().then(valid => {
        if (valid) {
          whisperingApi.addRelate(Object.assign({}, this.relate.entity, {
    ...
    release() {
      whisperingApi.sendPublic(this.entity.id).then(() => {
        this.success('发布成功')
        whisperingEventBus.$emit('list.update')
      })
    },
