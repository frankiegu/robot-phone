<template>
    <div>
                        ...
                        <Form class="panel-form"
                            ...
                            <FormItem label=""
                                ...
                                <i-select clearable
                                          ...
                                          style="width: 180px;"
                                          placeholder="选择代理类型">
                                    <Option v-for="(item,index) in proxyTypeList"
                                            :key="index"
                        ...
                        <div class="panel-header">
                            ...
                            <div class="panel-header-tools">
                                <i-button type="primary"
                                          icon="plus"
                                          @click="handlerAddProxy">开通市代</i-button>
                            </div>
                        </div>
                        <div class="panel-body"
                                    ...
                                    <div class="list-item">
                                        ...
                                        <div class="list-item-main">
                                            ...
                                            <div class="list-item-info">
                                                <ins :class="['medal',{'medal-gold':item.proxyType==3,'medal-sliver':item.proxyType==2,'medal-bronze':item.proxyType==1}]">{{item.proxyType|proxyType}}牌代理</ins> {{item.cityName}}
                                            </div>
                                        </div>
                                    </div>
                                </li>
                    ...
                    <div class="panel">
                        <div class="panel-header">
                            <i class="icon icon-enlarge panel-header-icon"
                               ...
                               @click="() => fullScreen = !fullScreen"></i>代理商详情
                            <div class="panel-header-tools"
                                 v-show="itemList.length">
                                <Button type="primary"
                                        @click="openBuyPort">购买端口</Button>
                                <Button type="primary"
                                        @click="openProxyUp">代理升级</Button>
                                <Button type="primary"
                                        @click="handlerEditProxy">编辑</Button>
                                <Button type="error"
                                        @click="deletProxy">删除</Button>
                            </div>
                        </div>
                        <div class="panel-body"
                             v-show="!itemList.length">
                        ...
                        <div class="panel-body"
                            ...
                            <Tabs value="1"
                                ...
                                <TabPane label="基础信息"
                                         name="1">
                                    <!-- 基础信息 -->
                                    <base-info :id="activedId"
                                               ref="baseInfo"></base-info>
                                </TabPane>
                                <TabPane label="企业列表"
                                         name="2">
                                    <!-- 企业列表 -->
                                    <firm-list v-if="tabName=='2'"
                                               :id="activedId"
                                               ref="firmList"></firm-list>
                                </TabPane>
                                <TabPane label="使用记录"
                                         name="3">
                                    <!-- 通话记录 -->
                                    <port-record v-if="tabName=='3'"
                                                 :id="activedId"
        ...
        <!-- 新增 代理-->
        <ms-panel v-model="newProxyShow"
                  title="开通代理">
            <new-proxy :visible="newProxyShow"
                       @after-save="afterAddProxySave"
                       @on-cancel="afterAddProxyCancel"></new-proxy>

        </ms-panel>

        <!-- 编辑 代理-->
        <ms-panel v-model="editProxyShow"
                  title="编辑代理">
            <edit-proxy :visible="editProxyShow"
                        :proxy-id="editProxyId"
                        @after-save="afterEditProxySave"
                        @on-cancel="afterEditProxyCancel"></edit-proxy>
        </ms-panel>
        <proxy-up v-model="proxyUpvisible"
                  :pObj="currentProxy"
                  @reload="proxyUpReload"></proxy-up>
...
export default {
    ...
    methods: {
        ...
        async getItemList() {
            let vm = this
            let result = await this.$pmsApi.cityProxy.getCityProxyList(vm.assignItemQuery(vm.query))
        ...
        deletProxy() {
            this.$Modal.confirm({
                title: `确认删除`,
                content: `删除${this.currentProxy.name}代理商会将此账号下级<span class='vma-wanming'> 市代、企业的数据一并删除，删除后 数据不可恢复！</span>`,
                onOk: (action, instance) => {
                    this.$pmsApi.cityProxy.deleteCityProxy(this.currentProxy.id).then(() => {
                        this.pageInit()
                        this.success("删除成功");
                    })
                }
