...
export default {
  ...
  methods: {
    ...
    async loadProvinceList() {
      this.provinceList = await this.$omsApi.cityProxy.listProvince()
    },
    async loadCityList(n) {
      if (n) {
        this.cityList = await this.$omsApi.cityProxy.getCityList(n)
      }
    },
    async loadProxyInfo() {
      let query = { cityId: this.entity.cityId }
      let proxyInfoList = this.proxyInfoList = await this.$omsApi.cityProxy.getProxyInfo(query)
    ...
    async handlerSave() {
      ...
      await this.$omsApi.cityProxy.addCityProxy(postData)
