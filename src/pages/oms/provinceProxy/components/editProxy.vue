<template>
  <div>
    <i-form :model="entity"
      ...
      <!-- <FormItem label="所属城市：">
        <i-input v-model="entity.cityName" disabled></i-input> 
...
export default {
  ...
  methods: {
    ...
    async loadDetail() {
      if (this.proxyId) {
        this.proxyDetail = await this.$omsApi.provinceProxy.getProvinceProxyDetail(this.proxyId)

        this.entity = Object.assign({}, this.entity, this.proxyDetail)
      }
    },
    handlerCancel() {
      this.$emit('on-cancel')
    ...
    async handlerSave() {
      ...
      await this.$omsApi.provinceProxy.editProvinceProxy(postData)
